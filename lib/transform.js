...
module.exports = class Transform {
    constructor(handleTags, baseTemplatesCacheSize) {
        this.handleTags = handleTags;

        if (baseTemplatesCacheSize) {
            this._bindAndMemoizeInternalMethods(baseTemplatesCacheSize);
        } else {
            this._bindInternalMethods();
        }
    }

    _bindAndMemoizeInternalMethods(baseTemplatesCacheSize) {
        const commonMemoizeOptions = {
            length: 1,
            max: baseTemplatesCacheSize
        };
