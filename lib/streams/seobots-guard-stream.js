...
module.exports = class SeobotStringifierStream extends stream.Transform {
     ...
     * @param {http.IncomingHttpHeaders} reqHeaders
     * @param {http.ServerResponse} res
     */
    constructor(enabled, reqHeaders, res) {
        super({ objectMode: true, autoDestroy: true });
        this.#isBot = enabled
            ? botDetector.parse(reqHeaders['user-agent']) !== null
            : false;
        this.#response = res;
    }
    ...
    _flush(done) {
        //Not allowing to send broken response to Bot
        if (this.#erroredState) {
            this.#batch = [];
            this.end();
            return;
